...
class ConcurrentStreamStateConverter(ABC):
    ...
    def is_state_message_compatible(state: MutableMapping[str, Any]) -> bool:
        return not state or state.get("state_type") in [t.value for t in ConcurrencyCompatibleStateType]
...
class IsoMillisConcurrentStreamStateConverter(ConcurrentStreamStateConverter):
    # TODO: add some analytics to indicate how far we've gotten before hitting an error, when an error is hit
    ...
    def _millis_format(self, timestamp: str) -> str:
        return pendulum.parse(timestamp).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
